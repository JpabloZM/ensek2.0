...
(function () {
    ...
    console.log(
        "%c[DEBUG] Sistema de monitoreo iniciado",
    ...
    window.setTimeout(function () {
        ...
        const summaryChartElement = document.getElementById(
            "inventorySummaryChart"
        );
        if (!summaryChartElement) {
            console.error(
                "[DEBUG] No se encontró el elemento canvas #inventorySummaryChart"
            );
        } else {
        ...
        setTimeout(function () {
            console.log(
                `[DEBUG] Resumen: ${window.errorCount} errores, ${window.warningCount} advertencias`
            );
        }, 5000);
    }, 1000);
})();
    ...
    setTimeout(function () {
        ...
        if (chartContainer) {
            if (chartContainer.innerHTML.trim() === "") {
                ...
                console.log(
                    "[DEBUG] chart-container parece tener contenido válido"
                );
            }
        }
    }, 2000);
