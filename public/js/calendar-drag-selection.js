...
document.addEventListener('DOMContentLoaded', function() {
        ...
        dragTimeout = setTimeout(() => {
            ...
            if (calendarContainer) {
                ...
                if (!calendarContainer.hasAttribute('data-original-width')) {
                    calendarContainer.setAttribute('data-original-width', calendarContainer.offsetWidth);
                }
            }
            ...
            let targetCell = elementUnderCursor.closest('.calendar-service-cell');
            if (!targetCell) return;
            
            // Verificar que la celda esté en la misma columna (mismo técnico)
            if (targetCell.getAttribute('data-technician-id') !== startCell.getAttribute('data-technician-id')) {
                return;
            }
    ...
    document.addEventListener('mouseup', function(e) {
        ...
        if (selectedCells.length === 1 && firstCell === startCell && firstCell === endCell) {
        ...
        document.querySelectorAll('.technician-calendar-container').forEach(container => {
    ...
    function selectCellsBetween(start, end) {
        ...
        const allCells = Array.from(document.querySelectorAll(`.calendar-service-cell[data-technician-id="${technicianId}"]`));
    ...
    function updateSelectionOverlay() {
        ...
        document.querySelectorAll('.technician-calendar-container').forEach(container => {
            ...
            const serviceCells = container.querySelectorAll('.calendar-service-cell[data-hour="0"]');
            if (!hourCell) return;
    ...
    function prepareModalData() {
        ...
        const endTimeStr = `${finalEndHour.toString().padStart(2, '0')}:${finalEndMinute.toString().padStart(2, '0')}`;
        ...
        openNewServiceModalWithRange(currentDate, startTimeStr, endTimeStr, technicianId);
     ...
     */
    function openNewServiceModalWithRange(date, startTime, endTime, technicianId) {
