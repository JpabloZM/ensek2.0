...
document.addEventListener("DOMContentLoaded", function () {
    ...
    window.syncTimeFields = function (
        ...
        console.log(
            `⚙️ Configurando sincronización para ${dateInputId}, ${endTimeInputId}, ${durationInputId}`
        );
        ...
        if (!dateInput || !endTimeInput || !durationInput) {
            console.error(
                "⚠️ No se encontraron todos los campos necesarios para sincronización:",
        ...
        const updatedStartTimeInput = startTimeInputId
        ...
        function updateTimeInfoBox(start, end, duration) {
            if (infoBox) {
                ...
                if (infoText) {
                    ...
                    console.warn(
                        "⚠️ Elemento de texto de tiempo no encontrado en infoBox"
                    );
                }
            } else {
                console.warn(
                    "⚠️ InfoBox no encontrado para actualización de tiempo"
                );
            }
        }
        ...
        function detectSpecialTimeRanges(startHour, endHour, durationMins) {
            console.log(
                `🔍 Analizando rango de tiempo: ${startHour}:00 - ${endHour}:00 (${durationMins} min)`
            );
            ...
            if (
                startHour === 8 &&
                endHour === 10 &&
                Math.abs(durationMins - 120) < 10
            ) {
                console.log(
                    "✓ Detectada selección especial: 8am a 10am (2 horas)"
                );
                return {
                    duration: 120,
                    description: "8:00 AM - 10:00 AM (2 horas exactas)",
                };
            }
            ...
            if (
                ...
                console.log(
                    `✓ Detectada selección especial: ${startHour}:00 a ${endHour}:00 (2 horas exactas)`
                );
                return {
                    duration: 120,
                    description: `${startHour}:00 - ${endHour}:00 (2 horas exactas)`,
                };
            }
            ...
            if (startHour === 8 && durationMins < 180) {
                console.log(
                    "✓ Detectado inicio a las 8:00 AM - sugiriendo 2 horas"
                );
                return {
                    duration: 120,
                    description: "8:00 AM - 10:00 AM (2 horas sugeridas)",
                };
            }
        ...
        updatedDurationInput.addEventListener("input", function () {
            console.log(
                `🕒 Evento: Cambio de duración a ${this.value} minutos`
            );
            // Obtener fecha y hora de inicio
            const dateValue = updatedDateInput.value;
            const startTimeValue = updatedStartTimeInput
                ? updatedStartTimeInput.value
                : null;
            ...
            if (startTimeValue) {
                ...
                console.log(
                    `📅 Fecha y hora de inicio separadas: ${dateValue}T${startTimeValue}`
                );
            } else {
                // Si es un único campo datetime-local
                startDateTime = new Date(dateValue);
                console.log(
                    `📅 Fecha y hora de inicio combinadas: ${dateValue}`
                );
            }
            ...
            const endDateTime = new Date(
                startDateTime.getTime() + durationValue * 60000
            );
        ...
        updatedEndTimeInput.addEventListener("input", function () {
            ...
            if (!isNaN(endHours) && !isNaN(endMinutes)) {
                console.log(
                    `🕒 Hora de fin analizada: ${endHours}:${endMinutes}`
                );
                ...
                if (endDateTime < startDateTime) {
                    console.log(
                        "⚠️ Hora fin anterior a inicio, ajustando a día siguiente"
                    );
                ...
                console.log(
                    `⏱️ Duración calculada: ${durationMinutes} minutos`
                );

                // Detectar casos especiales
                const specialCase = detectSpecialTimeRanges(
                ...
                if (specialCase) {
                    console.log(
                        `🔍 Caso especial detectado: ${specialCase.description}`
                    );
                    updatedDurationInput.value = specialCase.duration;

                    // Recalcular hora de fin para mantener consistencia
                    const recalcEndTime = new Date(
                        startDateTime.getTime() + specialCase.duration * 60000
                    );
                    const recalcHours = recalcEndTime
                        .getHours()
                        .toString()
                        .padStart(2, "0");
                    const recalcMinutes = recalcEndTime
                        .getMinutes()
                        .toString()
                        .padStart(2, "0");
                    ...
                    if (`${recalcHours}:${recalcMinutes}` !== this.value) {
                        console.log(
                            `📝 Ajustando hora de fin a ${recalcHours}:${recalcMinutes}`
                        );
                    ...
                    // Caso normal
                    console.log(
                        `📝 Aplicando duración estándar de ${durationMinutes} minutos`
                    );
                ...
                setTimeout(
                    () =>
                        updatedDurationInput.classList.remove(
                            "field-highlight"
                        ),
                    1000
                );
        ...
        function handleStartTimeChange() {
            ...
            if (!dateValue) {
                console.warn(
                    "⚠️ Valor de fecha vacío, abortando actualización"
                );
                return;
            }
            ...
            if (startTimeValue) {
                try {
                    ...
                    console.error(
                        `❌ Error al parsear fecha/hora: ${dateValue}T${startTimeValue}`,
                        e
                    );
                    return;
                }
                ...
                try {
                    ...
                    console.error(
                        `❌ Error al parsear fecha/hora: ${dateValue}`,
                        e
                    );
                    return;
                }
            ...
            if (startHour === 8 && startMinutes === 0) {
                console.log(
                    "🔍 Caso especial detectado: Inicio a las 8:00 AM - aplicando duración de 2 horas"
                );
                updatedDurationInput.value = 120;

                // Recalcular hora fin para 2 horas
                const specialEndTime = new Date(
                    startDateTime.getTime() + 120 * 60000
                );
                const specialEndHours = specialEndTime
                    .getHours()
                    .toString()
                    .padStart(2, "0");
                const specialEndMinutes = specialEndTime
                    .getMinutes()
                    .toString()
                    .padStart(2, "0");

                console.log(
                    `⏱️ Estableciendo hora de fin para caso especial: ${specialEndHours}:${specialEndMinutes}`
                );
                ...
                const durationValue =
                    parseInt(updatedDurationInput.value) || 60;
                console.log(
                    `📝 Usando duración estándar de ${durationValue} minutos`
                );
                ...
                console.log(
                    `⏱️ Calculando hora de fin: ${endHours}:${endMinutes}`
                );
                // Actualizar campo de hora fin
                updatedEndTimeInput.value = `${endHours}:${endMinutes}`;
        ...
        if (updatedStartTimeInput) {
            updatedStartTimeInput.addEventListener(
                "input",
        ...
        console.log(
            "🔄 Ejecutando sincronización inicial para configurar correctamente los campos"
        );
        handleStartTimeChange();

        console.log(
            `✅ Sincronización de campos establecida para ${dateInputId}, ${
                startTimeInputId || "N/A"
            }, ${endTimeInputId}, ${durationInputId}`
        );
    };
    ...
    document.addEventListener("shown.bs.modal", function (event) {
        ...
        if (event.target.id === "newScheduleModal") {
            console.log(
                "🔄 Modal de agendamiento abierto, inicializando sincronización"
            );
            setTimeout(() => {
                window.syncTimeFields(
                    "scheduled_date",
                    null,
                    "end_time",
                    "duration",
        ...
        // Si es el modal directo
            ...
            console.log(
                "🔄 Modal de agendamiento directo abierto, inicializando sincronización"
            );
            setTimeout(() => {
                window.syncTimeFields(
    ...
    document.addEventListener("reinitializeTimeSyncFields", function (event) {
        if (event.detail && event.detail.modalId) {
            console.log(
                `🔄 Evento personalizado: reinicializando campos para ${event.detail.modalId}`
            );
    ...
    window.addEventListener("load", function () {
        console.log(
            "🔄 Página cargada completamente, verificando modales abiertos"
        );
        ...
        if (scheduleModal && scheduleModal.classList.contains("show")) {
            console.log(
                "🔄 Modal de agendamiento encontrado abierto, aplicando sincronización"
            );
            window.syncTimeFields(
                "scheduled_date",
                null,
        ...
        if (directModal && directModal.classList.contains("show")) {
            console.log(
                "🔄 Modal directo encontrado abierto, aplicando sincronización"
            );
            window.syncTimeFields(
                "direct_scheduled_date",
                "direct_start_time",
